**殺人事件** のデータベースを掘り下げながら、**tidyr** と joinsという新しい概念を学びましょう。先に整理したデータについて簡単に述べましたが、このセクションで説明します。
...
データは面倒ですが、データをどのように積み重ねるかについての理想的な構造があります。
...
いろいろな型があります-- *解決済みかどうか*, *パーセント*,  *事件* が同じ列にあってはいけません。

Bしかし、この方式でソースからデータを取得したり、分析によってそのようなデータが生成されることがあります。

**殺人事件** データベースをもう一度見てみましょう。

データフレームを調べて、すべての変数（列）を検討するか[データ辞書](https://www.dropbox.com/s/lo6tgo8nnbpqeru/MAPdefinitionsSHR.pdf?dl=1)を調べます。

私たちが持っている変数に関して、どんな質問をすることができるでしょう？

* **MSA_label** は首都圏の統計領域です
* **VicRace_label** は人種です
    * Note: これはうのみにしてはいけません。 `murders %>% group_by(VicRace_label) %>% count()`を実行したら何が起きるでしょう？
    * 回答：「ヒスパニック系」の被害者のためのラベルはありません
* **Solved_label** は、事件が解決されたかどうかです。

このデータから、どの都市圏が他の地域よりも高い割合で特定の人種に対する殺人事件を解決しているか把握できます。ワシントンポストの [Where Killings Go Unsolved](https://www.washingtonpost.com/graphics/2018/investigations/where-murders-go-unsolved/) のようなものです。 私たちが扱っているデータはワシントンポストほど明確ではありません。彼らは緯度と経度のデータを持っているので、特定の場所の下で殺人事件群を識別することができるのです。私たちには郡や大都市圏に一般化されたデータがあります。

しかし、逮捕率の低い地域では未解決の殺人事件が暴力のサイクルが永続させる可能性があるため、問題となっている都市を特定するにはまだ十分です。
...
過去10年間の各都市圏での事件解決の割合を見つけることから始めましょう。
...
さて、これまでに事件数とそれらが解決されたかどうかを数えました。
...
よく見ると、このデータは乱雑ではありませんが、提示するのはそれほど簡単ではありません。

編集者が、どの主要都市圏が未解決事件の割合が最も高いか見たければどうしますか？

このデータフレームは簡単です。

* Fより小さいメトロエリアを排除するために10未満の事件があるケースを除外
...
繰り返しますが、編集者はあなたのたデータがきちんとしているか気にしません。

彼らが物語を見つけるために素早く分類できるものを与えましょう。
...
終わりに近づいています。しかし、人種は選別していません。この高いデータを変換して広くする必要があります。
...
さて、ここでいくつか「魔法」をお見せしましたが、何かを消すには不十分です。それを取り戻さなければなりません。

フォーマットで幅が広いデータがあり、それを分析や視覚化するため縦長にしたい場合はどうしますか？

長い（背の高い）形式でデータを構造化しようと試みるべき3つの理由があります:
...
再び使用したコードをコピーして貼り付け、 `select()` and `spread()` functions:
...
もともとの問題は、 **race_percent** が、 リストされたパーセンテージが有意であるかどうかを決定するためにどれだけのケースがあったかという情報を持っていなかったことでした。

2つの新しいデータフレーム **race_percent** と **race_cases** を作成しました。それぞれに必要なものが含まれています。

それでは、この2つをまとめてみましょう。
# データの接合

2つのデータセットを結合するには一方のデータセットの列と他方のデータセットの列、通常は2番目のデータセットの一部の行と最初のデータセットの一部の行を足し合わせます。

うまく結合するには、2つのデータセット間で一致するものが必要になります。それはキーです。
...
すべての結合で、2つの変数を渡します。最初の変数は目標とするデータフレームで、2番目の変数はあなたが持ってきたデータフレームです。デフォルトでは、関数は同じ列名で結合しようとします（複数の列名で結合することもできます）。結合する列を個別に指定することもできます。
...
うまくいきました！ そして重複を避けるために、列名は自動的に変更されています。
...
ジョージア州ダルトンとテキサス州のブラックの犠牲者に対する100％未解決の事件が大問題なのかどうか、疑問を呈したことを覚えていますか？被害者は１都市あたり1人だけになりました。こうして結果が歪められていたのです。
...
１行下のシカゴでも、黒人と白人の未解決の割合はそれぞれ68パーセントと53パーセントです。 バッファローで何が起こっているのでしょう？この地域の出身であるか、全国でこのような格差を探しているのであれば、これは報道に値するストーリーかもしれません。

データを結合するための他の方法を学ぶ必要があるにせよ、今は簡単な分析をしたい - これは既に　**dplyr**　で学んだことを使ってできます！
...
# 今回はby = "column_name"を使用する必要はありません。両方のデータフレームに、一致する列名が　Company の1つしかないためです。
