...
Thomas Hargroveは、Scripps Howard News Serviceの全国特派員で、FBIの殺人データを使用して、連続殺人を含む可能性が高い殺人事件が発生した地域を特定するアルゴリズムを開発しました。彼の仕事によってオハイオ州とインディアナ州の当局者は2010年の絞殺事件の調査を開始したのです。この事件は、Darren Deon Vannの逮捕につながりました。彼は数十年にわたって女性を殺害してきたと告白し、警察はこれらの未発見の犠牲者を見つけるため、インディアナ州ゲーリーの廃墟に向かうことになりました。
...
{{% notice warning %}}
これはデータ内の利用可能な変数に基づいたアルゴリズムです。有能で、捜査に熟練した警察の仕事には敵いません。このセクションで説明されている方法論は、無関係なケース間で誤った一致を作ったり、既知のリンクされた事件を検出できなかったりして、誤った結果を生み出す可能性があります。調査結果を報道する場合は、不必要な [過剰反応](https://www.youtube.com/watch?v=kdpG92dsx1A)を回避するためにさらなる調査が必須です。殺人事件を関連づけるべきかどうかについての権限は、犯罪を調査する地方の法執行機関および裁判所にあるのです。
{{% /notice %}}
...
郡の変数がありません。 郡のFIPSコード列はありますが、郡の名前を識別する列はないのです。
...
「キング」郡と「ワシントン」州でフィルタリングし、510件に絞り込みました。1982～2001年の間に発生し、犠牲者は女性の事件を探しました。
...
このデータセットで、私たちが抽出したデータがすべてゲイリー・リッジウェイの犠牲者であることを判断する方法がないことは明らかです。それぞれの犠牲者の事件ファイルを調べるか、彼の裁判で提示された証拠を精査することが必要でしょう。 しかし、彼の犠牲者のうちの何人かが上のリストにいるチャンスはあります。
### マインドハンター

探偵や犯罪プロファイラーのように考えてみましょう。

あなたは被害者のリストを絞り込み、シリアルキラーが逮捕を免れて殺害を続けている可能性のある地域に適用するプロセスをリバースエンジニアリングできますか。

データを飼い馴らす可能性があるパターンのリストとは何でしょうか？
...
これが、データを洞察するアプローチの方法を示すことを願っています。
