## ベクトル

**ベクトル**は、同じ基本型のデータ要素を順序付けた集まりです。ベクトルを構成する部分は、**コンポーネント**または**要素**と呼びます。

```{r vectors1}
vec1 <- c(1,4,6,8,10)
vec1
```

ベクトル `vec` は連結関数`c()`によって示されます。

```{r vectors2}
vec1[5]
```

ベクトルの要素は、 `[i]` と索引付けすることによって表示できます。

```{r vectors3}
vec1[3] <- 12
vec1
```

配列内の要素の一つが新しい数字に置き換えられています。

```{r vectors4}
vec2 <- seq(from=0, to=1, by=0.25)
vec2
```

これは、ベクトルを作成するもう一つの方法 `seq()` シーケンス関数を示しています。

```{r vectors5}
sum(vec1)
```


## 行列

**行列** は2次元ベクトルです。

こんな感じです。

```{r matrix}
mat <- matrix(data=c(9,2,3,4,5,6), ncol=3)
mat
```

引数 `data` は、どの数値が行列に含まれるべきかを指定します。

列を指定するには `ncol` 、行で指定するには `nrow` を使用してください。

行列演算はベクトル演算と大体同じです。

```{r matrix2}
mat[1,2]
```

行列の要素を扱うのに、特別な方法を覚える必要はありません。

```{r matrix3}
mat[2,1]
```

行全体を選択したい場合は、列番号の欄を空のままにします。列全体を選択するのはその逆です。

```{r matrix4}
mat[,3]
```


## データフレーム

Iスプレッドシートを使った作業に慣れている人は、Rでは *データフレーム* が最も扱いやすいでしょう。

Tこれは配列からデータフレームを作成する方法です。今完全に理解する必要はありません。スプレッドシートをインポートする場合、使用するデータは構造化されています。


```{r df1}
patientID <- c(111, 208, 113, 408)
age <- c(25, 34, 28, 52)
sex <- c(1,2,1,1)
diabetes <- c("Type1", "Type2", "Type1", "Type1")
status <- c(1,2,3,1)

patientdata <- data.frame(patientID, age, sex, diabetes, status)
patientdata
```

これが実際に起こっていることです。一連の配列が作成され`data.frame()` という関数がそれらを結合してデータフレーム構造にしているのです。

**データフレームから要素を取得する方法:**

```{r patientdata}
# a : means "through"
patientdata[1:2]

# So 1:2 means 1 through 2


patientdata[c("diabetes", "status")]

patientdata$age

patientdata[1:2]

patientdata[c(1,3),1:2]

patientdata[2:3, 1:2]
```

{{% notice tip %}}
 patientdata$age で列を参照することができますし、インデックスによって列を参照することもできます。この例ではインデックスは 2なので, patientdata[,2] とすれば同じ結果となります。3行目だけを取り出す場合は、 patientdata[3,]のようになります。 データ[行、列]と覚えてください。私はデータ[rocks]、つまりデータ[Ro,Cks]と暗記しています。
{{% /notice %}}

 `mean(patientdata[,2])`を使う代わりに, `$`を使用して`patientdata`から `age` を抽出できます。

```{r df2}
mean(patientdata$age)
```

これは `patientdata` の`age`を参照するための別の方法です。しかし、この方法はめったに使いません。


```{r df3}
mean(patientdata[["age"]])
```
## リスト

Rのもう一つの基本構造は *list*　です。

Tリストの最大の利点は、ベクトルの集まりである「列」（実際の順番として並んではいない）が、行列やデータフレームとは異なり、同じ長さでなくてもよいことです。

JSONファイルのようなものは構造化されています。

```{r list1}
g <- "My First List"
h <- c(25, 26, 18, 39)
# The line below is creating a matrix that's 5 rows deep of numbers 1 through(":") 10
j <- matrix(1:10, nrow = 5)
k <- c("one", "two", "three")
mylist <- list(title = g, ages = h, j, k)
```

こうすればリストをワークスペースに表示できます。

```{r list2}
names(mylist)
```

リストにあるものを見つける方法は

```{r list_extract}
mylist[[2]]

mylist[["ages"]][[1]]
```

上記のコードはリストからデータを抽出します。

```{r list3}
mylist$age + 10
```

リストの番号を参照し使用するには、このようにします。

## オブジェクトを操作するための関数

次のsample_df データフレームでやってみましょう。

```{r factor1}
# Run the lines of code below
sample_df <- data.frame(id=c(1001,1002,1003,1004), name=c("Steve", "Pam", "Jim", "Dwight"), age=c(26, 65, 15, 7), race=c("White", "Black", "White", "Hispanic"))
sample_df$name <- as.character(sample_df$name)
sample_df
```

`length(x)` - オブジェクトまたは配列に要素が何個あるかを調べる。

```{r length}
length(sample_df$name)
```

`nchar(x)` - **x** が文字列の場合、何文字あるかを調べる。

```{r nchar}
sample_df$name[1]
nchar(sample_df$name[1])
```

`dim(x)` - **x**　の列数と行数を表示します。
```{r dim}
dim(sample_df)
```

`ncol(x)` - 列数を数えます。

```{r ncol}
ncol(sample_df)
```

`nrow(x)` - **x**　の行数を返します。

```{r nrow}
nrow(sample_df)
```

`str(x)` - **x**　の行数を返します。

```{r str}
str(sample_df)
```

`summary(x)` - Rが理解しうる範囲でオブジェクトを要約します。

```{r summary}
summary(sample_df)
```


`View(x)` - RStudioで閲覧するためにオブジェクトを開くコマンドです。

```{r view, eval=F}
View(sample_df)
```

`rm(x)` -**x**　を削除します。

```{r rm, error=T}
rm(sample_df)
sample_df
```

## 演習

 [これらの演習](http://code.r-journalism.com/chapter-1/#section-data-structures-in-r) このセクションの知識を身につけましょう。

エクササイズアプリの実行に関する説明は、このセクションの [紹介ページ](http://learn.r-journalism.com/en/how_to_use_r/) にあります。

-----

<span style="color:gray">
