...
ここでは、FBIの補遺的な殺人レポートのケースレベルデータを使用します。 27,000人以上の殺人に関するデータがあり、非営利組織 [Murder Accountability Project](http://www.murderdata.org/p/data-docs.html)が情報自由法に基づき入手しました。
...
Bしかし、Rは（ある程度までの）ビッグデータを処理できます。データはコンピュータのメモリに保存されます。コンピュータのメモリが16ギガバイトなら、それがインポート可能な最大のファイルサイズです。Rの機能を実行するにはまだ多くのメモリを必要とするので、私はその数字にこだわることはお勧めしません。 ビッグデータを扱うならば、データをMySQLに入れるという方法があります。
...
{{% notice disclaimer %}}
データセットを結合するという概念を紹介するのはこれが初めてです。これは、データ分析において最も協力で重要なことの1つです。 [次の章](http://learn.r-journalism.com/en/wrangling/tidyr_joins/tidyr-joins/) で詳しく説明します。
{{% /notice %}}
...
## ID, CNTYFIPS, Ori, State, Agency, and AGENCY_A の各列を保持しています。
...
## ID, CNTYFIPS, Ori, State, Agency, and AGENCY_A の各列を削除します。
