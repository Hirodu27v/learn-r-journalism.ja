...
- CSVのインポートはベースRでできるので、本来パッケージは不要です。
...
CSVファイルをインポートするためのR関数は `read.csv()`です。 URLアドレスを引用符で囲み、  `stringsAsFactors=F` を追加します。(ここでは関数 `head()`を使用しています-- これはデフォルトで冒頭の6行を返しますが、10行見てみたいのでhead(data, 10)のように指定します。 )
...
**リンクのアドレスをコピー**　するのではなく、今度はリンクを右クリックしたとき--  **名前を付けてリンク先を保存**　をクリックします。

![](/importing_exporting/csv/images/downloading.png?classes=shadow)

作業ディレクトリに保存してください。

右側の更新矢印をクリックしてファイルを更新し、保存されていることを確認します。
...
ご覧のとおり、`read_csv()` 関数は **MonthYear** と **Town**  `read.csv()` のようにファクタ型としてではなくストリングとして解釈しています。
