...
* データの調査と変換に最適です。
* 特にパイプ演算子のチェーン構文を使用する際、直感的に読み書きできます。
...
私たちは何十年にも渡る殺人事件のビッグデータを集めています。

このケースレベルのデータは、司法省の [Murder Accountability Project](http://www.murderdata.org/p/data-docs.html) によって取得されました。

**dplyr** というVerb関数を使用してデータを分析し、追求すべきストーリーがあるかどうか確かめます。
...
{{% notice warning %}}
コマンドを実行する前に、スクリプトが作業ディレクトリのフォルダにあり、*data*　サブフォルダに **SHR76_16.sav.zip** があるようにしてください。 私のコンピュータには *import_murders.R* が *dplyr* 、 **SHR76_16.sav.zip** が *dplyr/data* に入っています。
{{% /notice %}}
 ...
 **import_murders.R** スクリプトは解凍されたデータをインポートし、それを実行可能なデータフレームに変換して、分析のためのオブジェクト **murders** として保存しました。 コンソールにいくつか警告が出ますが、問題ありません。
...
2つのベースR関数、 `unique()` と `length()`を使います。
...
新しいデータセットを入手したら、それが何であるのかを確認します。 そうすることで、どのように要約するか、またはそれを分解するかなんとなく分かるようになります。
...
それから、私が興味を持った変数にいくつの型があるのかを確認します。カテゴリカルなデータセットに含まれる変数（列）が多いほど、深い分析ができます。

たとえば、給与のリスト（1変数）を含むデータセットがある場合は、次のようにします。

* 平均給与を計算する
* 給与の最高額と最低額の差を計算する

給与と労働者の性別（2つの変数）のデータセットがある場合は、さらに次のことができます。:

* 男性と女性の平均給与を把握する
* 男女の中央値の差を計算する

給与と労働者の性別を含むデータセットがあり、労働者がどこに住んでいるかを明記している場合（3つの変数）、さらに次のことができます。:
...
{{% notice warning %}}
Rでは一つの  `=` は、 `<-`と同じく値を代入します。論理テストは二つの等号  `==`で等価かどうかテストします。
{{% /notice %}}
...
データフレームdf1とdf2はまったく同じです（夫が関与し、被害者が60歳を超え、2016年に発生した事例を検索しています） -  25件見つかりました。一方、d3には、夫またはボーイフレンドが関与していた、または三角関係の約32,000件の事件があります。
...
`group_by()` の対となる `ungroup()` があるということも後で必要かもしれないので指摘しておきます。
...
データ分析は、ほとんどの場合、複数のステップが必要です。

データセットを使ってこの複数のステップを処理する **オプション** を調べ、 *最良の* 方法を選びましょう。

 **murders** データフレーム上で実行する理論上のプロセス：

1. ワシントンDCで発生した事件の　**行を抽出** し
2. 年別に事件を　**まとめ**
3. 事件数を　**数え**
4. 事件を絞り込んで　**整理する**
...
```
 ...
 `%>%` は文法的に *それならば*　と同じといって良いのです。

おさらいですが:

* ワシントンDCで発生した事件の　**行を抽出** し
* 年別に事件を　**まとめ**
* 事件数を　**数え**
* 事件を絞り込んで　**整理する**
...
RStudioで `%>%` を入力するショートカットです。:

* Mac: Cmd + Shift + M
* Windows: Ctrl + Shift + M

なぜ「M」なのでしょう？きっとtefan Milton Bacheがこのパイプを [**magrittr** パッケージ](https://cran.r-project.org/web/packages/magrittr/README.html) で最初に導入したからなのでしょう。
...
たとえば、上のデータフレームでは、DCの殺人件数がわかりました。  `lag()` とちょっとした計算で、殺人件数の年ごとの変動を計算することができます。
...
`summarize()` dは、さっき `sum()` がやったように入力としてベクトルを受け取り、単一の値を出力します。
...
パーセンテージを計算することによって学んだことをまとめましょう。
...
とても興味深い。ここにあるデータの構造は、見慣れないものかもしれません。このデータはきちんと整えられています。各変数は列で、各観測値（または事件数）は行です。こうしておけば、Rの中のデータを分析してチャートを作成することが容易になります。

でも、あなたは以下のようなデータの方が見覚えがあるでしょう。
